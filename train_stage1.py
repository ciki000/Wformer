...
from losses import CharbonnierLoss,SSIMLoss, MixLoss
...
stage0_checkpoint = './log/Uformer32_1022_3_0/models/model_best.pth'
...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in enumerate(train_loader, 0): 
        ...
        loss0 = criterion0(reflex, target)
        loss1 = criterion1(restored, target)
        
        rest = opt.nepoch - epoch 
        if rest<opt.nepoch//3:
            loss = loss1
        else:
            loss =  (1 - epoch/opt.nepoch) *loss0 + 2*loss1
